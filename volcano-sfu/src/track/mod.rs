use std::{sync::atomic::Ordering, time::{SystemTime, UNIX_EPOCH}};
...
fn fast_forward_timestamp_amount(newest_timestamp: u32, reference_timestamp: u32) -> u32 {
    if is_timestamp_wrap_around(newest_timestamp, reference_timestamp) {
...
pub fn to_ntp_time(t: SystemTime) -> NtpTime {
    ...
    nsec = (nsec - sec * 1_000_000_000) << 32;
    let mut frac = nsec / 1_000_000_000;

    if nsec % 1_000_000_000 >= 1_000_000_000 / 2 {
        frac += 1;
    }
